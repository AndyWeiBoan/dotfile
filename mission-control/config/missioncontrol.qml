...
// A macOS-Mission-Control-style overview for Hyprland, in Quickshell.
//
// Runs as a daemon, started at login by autostart.lua, and shows/hides itself
// on `qs ipc call overview {toggle,show,hide}`. ~/.local/bin/missioncontrol
// wraps that (CTRL+UP and a 3- or 4-finger swipe up toggle, CTRL+DOWN closes).
//
// It is a daemon because launching it per keypress cost ~340ms before anything
// appeared, which is plainly visible as a lag. Measured breakdown: an empty
// layer surface is 5ms, so ~145ms was Qt/QML starting up and ~190ms was
// decoding Omarchy's 5120x2880 wallpaper. Neither is avoidable per launch --
// giving the Image a smaller sourceSize made it *slower* (505ms), and so did
// matching the decode size across the two Images that share the wallpaper
// (496ms). Staying resident pays both costs once, at login.
//
// This is also the pattern Quickshell's own FAQ prescribes: it ships no command
// to open or close a window, and points at IpcHandler flipping `visible`.
//
// Layout follows the real thing: a Spaces strip of desktop thumbnails across
// the top, and underneath it the *current* desktop's windows spread out so none
// overlaps, each with its app icon and title. Click a window to go to it, click
// a desktop in the strip to switch to it.
//
// Why this exists at all: the hyprexpo plugin does a similar job inside the
// compositor, but its background can only ever be a flat colour. hyprexpo is a
// render pass, not a client -- it clears the whole monitor to bg_col every
// frame, so a transparent bg_col still paints black and nothing can show
// through from underneath, and wallpaper_bg = 1 draws Hyprland's *built-in*
// wallpaper, which Omarchy does not use (its wallpaper is painted by a
// layer-surface client the plugin cannot reach). Both were measured, not
// assumed. Owning our own surface is the only way to get the frosted wallpaper
// that makes this read as macOS. See ~/dev/dotfile/mission-control/FINDINGS.md.
//
// The premise -- that a window sitting on a workspace you cannot see can still
// be captured -- was verified before any of this was written: Hyprland renders
// a toplevel into an offscreen buffer on demand for screencopy, so visibility
// does not matter and the thumbnails are live, not stale.
//
// hyprexpo is still installed and still configured in looknfeel.lua. To go back
// to it, point the CTRL+UP bind and the swipe-up gestures at
// hl.plugin.hyprexpo.expo("toggle") again; nothing here has to be removed.
...
ShellRoot {
  ...
  // Starts HIDDEN. The daemon is started at login, and a config that showed
  // itself on load flashed the whole overview across the screen on every boot
  // before the wrapper's `hide` landed. The wrapper's cold-start path calls
  // `show` explicitly, so a keypress that finds no daemon still opens.
  property bool shown: false

  // Second phase of the open. `shown` puts the surface up with every window
  // still drawn at its real size and position -- which looks exactly like the
  // desktop you were already on -- and `expanded` is what shrinks them into the
  // overview. Splitting them is the whole trick behind the macOS feel: the
  // desktop appears to shrink, rather than a different-looking screen fading
  // in over it. They cannot be one flag, because the windows have to be painted
  // at full size for at least one frame before the animation starts from there.
  property bool expanded: false

  // What the user last asked for, as opposed to what is currently on screen.
  // Needed because the open is asynchronous: a timer fires later to start the
  // shrink, and if the overview was closed in the meantime that timer must not
  // resurrect it. Guarding those timers on `shown` was not enough -- during the
  // 260ms close, `shown` is still true, so a pending expand timer would set
  // `expanded` back to true and leave it stuck true while hidden. `toggle` then
  // read that as "already open" and tried to close an already-closed overview,
  // which is why CTRL+UP stopped working while `open` still did.
  ...
  //
  // Animating it out was the flicker: that band of screen then changed twice in
  ...
  IpcHandler {
    ...
    // listen, prop -- and its argument parser claims those words wherever they
    // appear, so `qs ipc call overview show` never reaches this object: it
    // prints the target's function list and exits 0. Silently, which is worse
    // than failing. `hide`, `toggle` and `state` are not subcommand names and
    // work; avoid the five that are.
    function open(): void { root.setShown(true); }
    function hide(): void { root.setShown(false); }
    // Keyed on `expanded`, not `shown`: during the closing animation `shown` is
    // still true, so a toggle then would try to close something already
    // closing and appear to do nothing.
    // Keyed on the intent, not on what is on screen: mid-animation neither
    // `shown` nor `expanded` answers "should this be open?" correctly.
    function toggle(): void { root.setShown(!root.wantOpen); }
  ...
  function setShown(next) {
    ...
    if (next) {
      // The daemon has been idle, possibly for hours. Hyprland's model is
      ...
      // Cheap enough to keep in the critical path (see the timing note by the
      // ScreencopyViews) and the alternative -- showing first and correcting
      // after -- would move windows under the pointer.
  ...
  Timer {
    id: expandFallback
    interval: 400
    onTriggered: if (root.wantOpen) root.expanded = true
  }

  // Hide, never Qt.quit(): this process is the daemon, and quitting it would
  // mean the next keypress pays the full startup cost again.
  ...
  Timer {
    ...
    onTriggered: if (!root.wantOpen) root.contentVisible = false
  }

  Timer {
    id: collapseThenHide
    interval: root.shrinkDuration - 60 + root.fadeDuration
    onTriggered: if (!root.wantOpen) root.shown = false
  }

  Timer {
    id: hideSoon
    interval: 80
    onTriggered: root.setShown(false)
  }
  ...
  Variants {
    ...
    PanelWindow {
      ...
      onBackingWindowVisibleChanged: {
        if (backingWindowVisible && root.wantOpen)
          firstFrame.start();
        else
          firstFrame.stop();
      }
      ...
      Item {
                  ...
                  Item {
                    ...
                    Repeater {
                      ...
                      delegate: ScreencopyView {
                        ...
                        x: (ipc.at[0] - panel.monX) * k
                        y: (ipc.at[1] - panel.monY) * k
                        width: ipc.size[0] * k
                        height: ipc.size[1] * k

                        captureSource: modelData.wayland
                        // Live, but only while shown. A resident daemon left
                        // with `live: true` would keep pulling frames of every
                        // window on every workspace forever, for a view nobody
                        // is looking at.
                        //
        ...
        Repeater {
          ...
          delegate: Item {
            ...
            readonly property real screenX: ipc.at[0] - panel.monX
            readonly property real screenY: ipc.at[1] - panel.monY

            readonly property real realX: screenX
            readonly property real realY: screenY
            readonly property real realW: ipc.size[0]
            readonly property real realH: ipc.size[1]
